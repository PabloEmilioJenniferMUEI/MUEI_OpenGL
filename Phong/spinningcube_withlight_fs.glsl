...
vec3 CalcPointLight(Light light, vec3 view_dir){
	...
	vec3 diffuse = light.diffuse * diff * vec3(texture(material.diffuse, vs_tex_coord)); //diapo 12
	...
	vec3 specular = light.specular * spec * vec3(texture(material.specular,vs_tex_coord)); //diapo 13
